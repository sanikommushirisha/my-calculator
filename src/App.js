...
function App() {
  ...
  const onKeyClick = (newValue, keyType) => {
    switch (keyType) {
      case "fx":
        onFunctionKeyClick(newValue);
        break;
      case "numeric":
        onNumericKeyClick(newValue);
        break;
      case "operator":
        onOperatorClick(newValue);
        break;
    }
  };
  ...
  const onDisplayInputChange = (updatedDisplayValue) => {
    const updatedAccVal =
      NumberUtils.removeLastNChar(accValue, displayValue.length) +
      updatedDisplayValue;
    setDisplayValue(updatedDisplayValue);
    setAccValue(updatedAccVal);
  };
